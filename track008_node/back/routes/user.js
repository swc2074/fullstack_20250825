...
const {
    createUser, getAllUsers, updateUserNickname,deleteUser
 ...
 *   post:
 *     summary: 로그인 (테스트) 
 *     description: Passport LocalStrategy 이메일/ 비밀번호 인증후 세션저장 이메일과 비밀번호를 받아서 단순 응답을 반환합니다.  (Passport 미적용)
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *       401:
 *         description: 로그인 실패 (테스트용)
 */
// /user/login
router.post('/login', async(req, res, next)=>{
     //실행 → passport/local.js, LocalStrategy 호출
    passport.authenticate('local',(err,user,info)=>{ // err 인증과정발생에러, 성공반환된사용자객체,실패메시지 객체
        if(err) return next(err);
        if(!user) return res.status(401).json({message:info?.message || '로그인 실패'});

        req.login(user, (loginErr)=>{ // passport 가 제공하는 함수, 인증된 사용자객체를 세션에저장하는 과정
 ...
 *   post:
 *     summary: 로그아웃 (테스트용)
 *     description: Passport 세션 기반으로 세션 종료 세션/쿠키 없이 단순 응답 반환
 *     responses:
 *       200:
 *         description: 로그아웃 성공
 */
router.post('/logout',isAuthenticated, (req, res)=>{ // isAuthenticated 로그인한 유저만다음진행, x 401
    ...
    req.logout((err)=>{ //passport가 제공하는 함수, 현재 로그인된 사용자정보를 세션에서 제거
        ...
        req.session.destroy(()=>{ // 세션을 제거
            res.clearCookie('connect.sid'); // 브라우저에 저장된 세션쿠키 삭제 , 쿠키가 없으면 브라우저가 세션식별 못함.
 ...
 *   get:
 *     summary: 전체 사용자 조회
 *     description: 로그인되 사용자만 전체 사용자 목록을 조회 할 수 있습니다. 모든 사용자의 목록을 반환합니다. (인증 미적용)
 *     responses:
 *       200:

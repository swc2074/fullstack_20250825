📂 프로젝트 주요 구성
1. src/main/java
- controller (com.theja703.controller)
- AppUserController.java
→ 웹 요청을 받아서 서비스 계층으로 전달하는 역할. MVC 패턴에서 C(Controller) 부분입니다.
- dao (com.theja703.dao)
- AppUserDao.java, MbtjDao.java, TestDao.java
→ DB와 직접 연결되는 인터페이스. MyBatis 매퍼 XML과 연결되어 SQL을 실행합니다.
- service (com.theja703.service)
- AppUserService.java, AppUserServiceImpl.java
→ 비즈니스 로직을 담당. DAO를 호출해서 데이터를 처리하고, Controller에 결과를 반환합니다.
- dto (com.theja703.dto)
- AppUserDto 등
→ DB 테이블과 매핑되는 데이터 객체. Controller ↔ Service ↔ DAO 간 데이터 전달에 사용됩니다.

2. config (설정 파일)
- appuser-mapper.xml → MyBatis SQL 매퍼 (insert, update, delete, select 쿼리 정의)
- db.properties → DB 연결 정보 (URL, 계정, 비밀번호 등)
- mybatis-config.xml → MyBatis 전역 설정 (환경, 매퍼 등록 등)
- root-context.xml → Spring Bean 설정 (Service, DAO 등)
- servlet-context.xml → DispatcherServlet 설정, ViewResolver 등
- test-mapper.xml → 테스트용 매퍼

3. src/test/java
- Test1_Board.java
→ JUnit 기반 테스트 코드. DAO/Service/Controller 동작을 검증합니다.

4. src/main/resources
- log4j2.properties, log4jdbc.log4j2.properties
→ 로깅 설정. SQL 실행 로그를 보기 위해 log4jdbc 사용.

5. webapp (View 계층)
- view/inc → 공통 JSP (header.jsp, footer.jsp)
- view/member → 회원 관련 JSP (join.jsp, list.jsp, edit.jsp, delete.jsp)
- META-INF → 메타데이터

6. 라이브러리/환경
- JRE System Library [JavaSE-11] → Java 11 사용
- Apache Tomcat v9.0 → WAS (웹 애플리케이션 서버)
- Maven Dependencies → Spring, MyBatis, JDBC, Log4j 등 의존성 관리
- JUnit 4 → 단위 테스트 프레임워크

⚙️ 전체 동작 흐름 (MVC + MyBatis)
- 사용자 요청 (JSP → Controller)
예: 회원가입 폼 제출 → AppUserController로 전달
- Controller → Service 호출
Controller는 요청을 Service로 넘겨 비즈니스 로직 실행
- Service → DAO 호출
Service는 DAO를 통해 DB 작업 수행
- DAO → MyBatis 매퍼 XML 실행
SQL 실행 후 결과 반환
- Service → Controller → JSP(View)
결과를 다시 Controller로 전달하고, JSP에 렌더링

✨ 요약
- Controller: 요청 처리
- Service: 비즈니스 로직
- DAO + Mapper XML: DB 접근
- DTO: 데이터 전달 객체
- JSP(View): 화면 출력
- Config: Spring/MyBatis 환경 설정

👉 이 구조는 Spring MVC + MyBatis + JSP의 전형적인 패턴입니다.
혹시 원하시면 제가 회원가입 흐름이나 로그인 흐름을 실제 코드 기준으로 단계별로 설명해드릴 수도 있어요. 원하시는 흐름을 말씀해주실래요?


🧩 회원가입 전체 흐름
1. 사용자 입력 (View, JSP)
- 사용자가 join.jsp 화면에서 이메일, 비밀번호, MBTI 타입 등을 입력하고 제출합니다.
- <form> 태그를 통해 POST /appuser/join 같은 URL로 요청이 전달됩니다.

2. Controller (AppUserController)
- AppUserController가 해당 요청을 받아서 DTO(AppUserDto) 객체에 폼 데이터를 바인딩합니다.
- Controller는 Service 계층을 호출합니다:
@PostMapping("/join")
public String join(AppUserDto dto) {
    appUserService.insert(dto);
    return "redirect:/member/list";
}



3. Service (AppUserServiceImpl)
- 비즈니스 로직을 담당하는 Service가 DAO를 호출합니다.
- 예: appUserService.insert(dto) → 내부에서 appUserDao.insert(dto) 실행
- 여기서 중복 이메일 체크 같은 로직을 추가할 수도 있습니다.

4. DAO (AppUserDao + Mapper XML)
- DAO 인터페이스는 MyBatis 매퍼 XML과 연결됩니다.
- appuser-mapper.xml의 <insert> 구문이 실행됩니다:
<insert id="insert" parameterType="AppUserDto">
  INSERT INTO appuser (APP_USER_ID, EMAIL, PASSWORD, MBTI_TYPE_ID, CREATED_AT)
  VALUES (appuserid_seq.nextval, #{email}, #{password}, #{mbtiTypeId}, #{createdAt, jdbcType=TIMESTAMP})
</insert>



5. DB (Oracle)
- appuser 테이블에 새로운 레코드가 추가됩니다.
- APP_USER_ID는 시퀀스(appuserid_seq.nextval)로 자동 증가.
- EMAIL은 UNIQUE 제약조건이 걸려 있을 수 있으므로, 중복 시 ORA-00001 오류 발생.

6. 결과 반환
- DAO → Service → Controller로 성공 여부가 전달됩니다.
- Controller는 성공 시 회원 목록 페이지(list.jsp)로 리다이렉트하거나, 실패 시 에러 메시지를 보여줍니다.

⚙️ 흐름 요약
- join.jsp → 사용자 입력
- AppUserController → 요청 처리, DTO 생성
- AppUserServiceImpl → 비즈니스 로직
- AppUserDao + appuser-mapper.xml → SQL 실행
- Oracle DB → 데이터 저장
- 결과 JSP → 성공/실패 화면 출력

👉 이 구조에서 자주 발생하는 문제는:
- EMAIL 중복 → DuplicateKeyException
- createdAt이 null인데 jdbcType 미지정 → JdbcType OTHER 오류
- 잘못된 컬럼명(id vs app_user_id) → ORA-00904 invalid identifier




#6. Upload
  1. model
    >table:이미지 경로필드

    desc sboard1;
    Name                                      Null?    Type
 ----------------------------------------- -------- ----------------------------
 ID                                        NOT NULL NUMBER
 APP_USER_ID                               NOT NULL NUMBER
 BTITLE                                    NOT NULL VARCHAR2(1000)
 BCONTENT                                  NOT NULL CLOB
 BPASS                                     NOT NULL VARCHAR2(255)
 BFILE                                              VARCHAR2(255)
 BHIT                                               NUMBER(10)
 BIP                                       NOT NULL VARCHAR2(255)
 CREATED_AT                                         TIMESTAMP(6)

 > dto: bfile
 > dao: insert, update

----------------------
	<insert id="insert2"
		parameterType="Sboard1Dto">
		
		insert into sboard1(ID , APP_USER_ID, BTITLE, BCONTENT, BPASS,BFILE,BIP)
    values (sboard1_seq.nextval , #{appUserId}, #{btitle}, #{bcontent}, #{bpass},#{bfile} #{bip})
	</insert>
	----------------------

  -----------------------
	<update  id="update2"  parameterType="Sboard1Dto">
	update sboard1
        set BTITLE= #{btitle}, BCONTENT=#{bcontent}, BFILE=#{bfile}
        where id=#{id} and BPASS=#{bpass}	
	</update>
    -----------------------


 2. view
    form: encType = "multipart/form-data"  
      method="post"
      <input type="file" />

 3. controller

     pom.xml
              <!-- img upload   start -->
              <!-- img upload   start -->
              <!-- https://mvnrepository.com/artifact/commons-fileupload/commons-fileupload -->
              <dependency>
                  <groupId>commons-fileupload</groupId>
                  <artifactId>commons-fileupload</artifactId>
                  <version>1.3.1</version>
              </dependency>
              <!-- img upload end -->q
              <!-- img upload end -->      

     servlet-context.xml
     FilUpload.java
     Controller.java - service



7. Ajax

1. 글 검색
2. 아이디중복검사
3. 유저 수저, 삭제

S
1) model
table, dto, dao#, service
select * from emp  where like '%S%';

>table - sql: 해당아이디가 있는지 검색 - sql 작성  appuser
  select * from appuser  where email='1@1';
>dto:
>dao:



2) controller
@RestController  //@Controller + @ResponseBody
http://localhost:8282/spring005/selectSearch?search=t
2) view (ajax)


---------------------
#8. AOP + Mybatis
---------------------
1. AOP 개요
STEP1)
- 핵심기능 : (비지니스로직)
- 부가기능 : 핵심기능(비지니스로직)을 도와줌
>> AOP - 부가기능

  @Controller    /list.do
          ■ 부가기능                    ■ 부가기능
         [시작]-  Service  (전체리스트뽑아주는 service) -[끝]
             |  처리뷰   
             ↓
         [시작]- Repository(전체리스트뽑아주는 sql   )  -[끝]


STEP2) Aspect
- 기능분리
- 런타임 시 필요한 위치에 동적으로 처리
        Advice + PointCut = Aspect
  부가기능정의   적용부위

STEP3) 용어
Target   -  대상
Advice   -  부가기능
PointCut -  적용할 타겟의 메서드를 선별하는 정규식
Weaving  -  Target에 부가기능 처리(삽입)하는 과정


STEP4) 
<<BEFORE>>         
application         
[
 [  비지니스로직    ]
 [  보안, 인증, 로직]
]

<<AFTER>>
application         
[
 **********************           
 *[   PointCut 적용부위 ]    Weaving  →    [  비지니스로직    ]
        ↑
 *[   Advice 부가기능   ] *
       ↑
 *[  보안, 인증, 로직   ] *
 *   [Aspect]
 **********************  
]
Target   -  대상
Advice   -  부가기능
PointCut -  적용할 타겟의 메서드를 선별하는 정규식
Weaving  -  Target에 부가기능 처리(삽입)하는 과정


2. AOP 특징
>> 프록시 기반 aop를 지원
- 프록시는 advice(부가기능)을 target(대상객체)에 적용하면서 생성되는 객체
- Target(대상객체)를 감싸는 프록시는 런타임 시에 생성됨.
- 타겟객체에 대한 호출을 intercept(가로채서)해서  
  A. 부가기능 수행 후 타겟의 핵심기능 로직호출(전처리)  , 
  B. 타겟의 핵심기능 로직호출  부가기능 수행  (후처리) 
  

   호출 --->   [ Proxy  [Target]  ]

3. 구현방식
1) xml
2) annotation

실습1) pom.xml
1. AspectJ runtime   -> AspectJrt
2. AspectJ weaver    -> AspectJweaver
3. Spring aop        -> spring-aop
4. api 문서
https://www.eclipse.org/aspectj/doc/next/runtime-api/

<!-- https://mvnrepository.com/artifact/org.aspectj/aspectjrt -->
<dependency>
    <groupId>org.aspectj</groupId>
    <artifactId>aspectjrt</artifactId>
    <version>1.7.3</version>
    <!-- <scope>runtime</scope>  -->
</dependency>
<!-- https://mvnrepository.com/artifact/org.aspectj/aspectjweaver -->
<dependency>
    <groupId>org.aspectj</groupId>
    <artifactId>aspectjweaver</artifactId>
    <version>1.7.3</version>
    <!-- <scope>runtime</scope> -->
</dependency>
<!-- https://mvnrepository.com/artifact/org.springframework/spring-aop -->
<dependency>
    <groupId>org.springframework</groupId>
    <artifactId>spring-aop</artifactId>
    <version>4.3.28.RELEASE</version>
</dependency>



실습2)  Advice + PointCut = Aspect
  부가기능정의   적용부위

com.company.aop_xml

>> AopTrace.java


import org.aspectj.lang.ProceedingJoinPoint;

public class AopTrace {
   public Object trace( ProceedingJoinPoint  joinPoint)  throws Throwable{
      // 타겟메서드의 정보
      String signature = joinPoint.getSignature().toShortString();
      System.out.println(">>>> " + signature + " START! ");
      // 타겟메서드 호출시간확인
      long start = System.currentTimeMillis();
      // 타겟메서드 호출
      Object  result = joinPoint.proceed();
      long end  = System.currentTimeMillis();
      System.out.println("..... 실행시간 : " + (end - start)  + "ms");
      System.out.println(">>>> " + signature + " END! ");
      return result;
   }
}


실습3)  servlet-context.xml
<?xml version="1.0" encoding="UTF-8"?>
<beans xmlns="http://www.springframework.org/schema/beans"
   xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
   xmlns:aop="http://www.springframework.org/schema/aop"
   xsi:schemaLocation="http://www.springframework.org/schema/beans    http://www.springframework.org/schema/beans/spring-beans.xsd
   http://www.springframework.org/schema/aop http://www.springframework.org/schema/aop/spring-aop-4.3.xsd">
   
   <aop:config>
      <aop:aspect  id="traceAspect"   ref="aopTrace" >
         <aop:around     method="trace"  
                    pointcut="execution(public * com.company.service..*(..))"
          />
      </aop:aspect>
   </aop:config>
   
   <!-- Advice 클래스 Bean 등록 -->
   <bean  id="aopTrace"   class="com.company.aop_xml.AopTrace" />
</beans>



■ MYBATIS
> 동적SQL 


select * from emp;
select * from where ename='Scott';
select * from emp where job='SALESMAN';

SQL 구문) MYBATIS 3개

<select id="findActiveBlogWithTitleLike"
     resultType="Blog">
  SELECT * FROM BLOG
  WHERE state = ‘ACTIVE’
  <if test="title != null">
    AND title like #{title}
  </if>
</select>

Q1> select * from emp where ${searchType} = #{keyword}
select * from where ename='Scott';
select * from emp where job='SALESMAN';

${searchType}  -> ''안붙음 stmt
#{keyword}  -> 자동 ''    pstmt



▶ 1. Security?
 - 애플리케이션의 보안(인증, 인가)를 담당
 - filter 흐름에 따라 처리

 ▶ 2. 인증 vs 인가
 - 인증 Authentication    [본인]이 맞는지 확인 
 - 인가 Authorization   인증된 사용자가 [접근가능]


▶ 3. Security 아키텍쳐
=====================
       ② [UsernamePasswordAuthentication Token]
          ↓
① Http Request  →     [AuthenticationFilter] ③ ...  →  [Authentication  Manager]
         ↓⑩               ⑨     ←
          [SecurityContextHolder]
=====================

- 1. 사용자가 로그인 폼태그 시도(username + password 전달)
- 2. ★ UsernamePasswordAuthentication  요청정보  Authentication 을 생성
- 3. Authentication 인증처리
                [Authentication  Manager]

- 10. 인증완료[사용자정보]  SecurityContextHolder담기
   - AuthenticationSeccessHandler 를 실행(성공)
   - AuthenticationFailureHandler 를 실행(실패)



   =====================
       [Authentication  Manager]  ->     [Authentication  Provider(s)]
   =====================

   - 4. Authentication  Provider(s) 인증담당.
    1) UsernamePasswordAuthentication   token 은 provider를 찾는데 사용
    2) AuthenticationProvider   ★로그인 정보  db 정보와 비교
    3) UserDetailsService       ★db에 있는 [사용자정보] 가져오기


    [실습]
    1. pem.xml  - security
    2. security-context.xml
    3. web.xml  스프링구동
    4. SercurityController
    5. 테스트

[실습1]  DB연동
1. security-context.xml  <bean id = "" class="BCrypt"/>
2. appuser-mapper.xml   / AppUserDao / mybatis-config.xml (Dto)
   1) 권한 삽입 
   insert into  authorities  (email, auth)  values ('1@1' , 'ROLE_MEMBER') 
   insert into  authorities  (email, auth)  values ('1@1' , 'ROLE_ADMIN') 

   2) email, password, 권한
   select     u.email, password, auth
   from       appuser  u  left join  authorities a  on  ( u.email = a.email )
   where      u.email = '1@1' 

   3) 회원가입 insert
    test




[실습3]  로그인
1. security-context.xml
   로그인폼
   로그인성공
   로그인실패
   유저정보
2. 설정내용  com.thejoa703.security
3. Controller - 로그인
4. view  - login.jsp


    ## 1. pom.xml
      <!-- SECURITY -->
      <!-- SECURITY -->
      <!-- SECURITY -->
      <!-- https://mvnrepository.com/artifact/org.springframework.security/spring-security-core -->
      <dependency>
         <groupId>org.springframework.security</groupId>
         <artifactId>spring-security-web</artifactId>
         <version>4.2.2.RELEASE</version>
         <!-- <version>5.0.7.RELEASE</version> -->
      </dependency>
      <dependency>
         <groupId>org.springframework.security</groupId>
         <artifactId>spring-security-config</artifactId>
         <version>4.2.2.RELEASE</version>
      </dependency>
      <dependency>
         <groupId>org.springframework.security</groupId>
         <artifactId>spring-security-core</artifactId>
         <version>4.2.2.RELEASE</version>
      </dependency>
      <dependency>
         <groupId>org.springframework.security</groupId>
         <artifactId>spring-security-taglibs</artifactId>
         <version>4.2.2.RELEASE</version>
      </dependency>
      <!-- SECURITY -->
      <!-- SECURITY -->
      <!-- SECURITY -->


------------------------------
#12. project
------------------------------
Q1. project2 프로젝트 만들기
   1. dynamic web project- project2
   2. cofigure - [Convert to Maven Project]
   3. spring - add Spring project Nature
   4. java se-11/ project facts, build path
   

프로젝트 진행 

   발표 12월 8일 


   ◎1일차 20251201
   1. 점검사항
   >> 1. sql 점검
   >> 2. 피그마 확인
   >> 3. 외래키 - erd

   4. 포트폴리오  깃허브올리기  (Basic - 기본설정)
   0. 방법
       package - com.project.controller/ com.project.dao/com.project.service
       package - com.project.users / com.project.board

       1. 포트폴리오 프로젝트: 팀원들 내려받기
       2. sql 파일도 같이 올려서 (공통의  db사용)

   >> 5. 12:30 pr 시간
   >> 6. 17:30 pr 시간


   ◎2일차 20251202
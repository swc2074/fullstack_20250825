...
class IceCreamDTO{
	...
	public String toString() {
		return "IceCreamDTO [name=" + name + ", price=" + price + "]";
	}
...
public class ListEx003 {
	public static void main(String[] args) {
		...
		while (true) {
			...
			switch (choice) {
			case 1:
				System.out.println(" 1. 아이스크림 추가 기능.");
				System.out.println("아이스크림 이름:");
				String name = scanner.next();
				System.out.println("아이스크림가격 입력");
				int price = scanner.nextInt();
				list.add(new IceCreamDTO(name,price)); // ##
				System.out.println("[" + name + "]" + "추가 완료!");
				break;
			case 2: 
				System.out.println(" 2. 아이스크림 목록 보기");
					if(list.size() == 0) {
						System.out.println("아직 등록된 아이스크림이 없습니다."); break;
						//※list.isEmpty() 비어있는지 확인
						for(IceCreamDTO ice : list) {System.out.println("-" + ice);
					} // toString
			...
			case 3: 
				...
				for(int i=0; i<list.size(); i++) {
					if (list.get(i).getName().equals(rename)) {find=i; break;}
				}
				list.remove(find);
				System.out.println("-" + rename + "삭제 완료!");
				break;
				
			case 4: 
				
				System.out.println(" 검색할 아이스크림 이름:");
